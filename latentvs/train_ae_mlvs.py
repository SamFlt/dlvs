...
from datasets.AEDataset import StoredLookAtAEDataset, StoredLookAtAEDatasetWithNearestNeighbors
...
def train(config, make_model_name):
    ...
    use_ae_loss, use_ml_dct_loss, use_filter_loss,\
        use_pose_loss, use_pose_im_loss, use_im_smoothing_unsup_loss, use_im_triangular_sup_loss = use_losses
    ...
    compute_interaction_matrices = use_pose_im_loss or use_im_smoothing_unsup_loss or use_im_triangular_sup_loss 
    compute_rec = use_ae_loss

    # Model creation
    model = None
    if config['model_type'] == 'resnet':
        model = ResNetAEIMComputable(latent_dim=latent_dim, training_vae=False, input_image_size=train_image_size, **config['model_params'])
    ...
    dataset = StoredLookAtAEDatasetWithNearestNeighbors(batches_per_epoch, batch_size, select_k_nn=3, distance_metric='se3',
    ...
    # dataset = TranslationDataset(batches_per_epoch, batch_size, num_workers, config['generator_params'], translation_ranges=tr)
    # dataset = FourDOFDataset(batches_per_epoch, batch_size, num_workers, config['generator_params'], translation_ranges=tr, max_rz_deg=rz_range)
    ...
    val_dataset = StoredLookAtAEDatasetWithNearestNeighbors(batches_per_epoch // 2, batch_size,  3, 'se3', num_workers,
                                                            config['val_generator_params'], config['dataset_params'], 
                                                            rz_max_deg=config['dataset_params']['rz_max_deg'])
    
    # val_dataset = TranslationDataset(batches_per_epoch // 2, batch_size, num_workers, config['val_generator_params'], tr)
    # val_dataset = FourDOFDataset(batches_per_epoch // 2, batch_size, num_workers, config['val_generator_params'], tr, rz_range)
    ...
    def do_iter(epoch, batch_idx, batch, is_train):
        ...
        ae_loss, ml_dct_loss, filter_loss, pose_loss, \
            pose_im_loss, im_smooth_unsup_loss, im_triangular_sup_loss = [torch.tensor(0, device=device) for _ in range(len(use_losses))]
        ...
        if use_im_smoothing_unsup_loss:
            im_smooth_unsup_loss = jacobian_similiarity_distance_weighted(Lz, z)

        if use_im_triangular_sup_loss:
            im_triangular_sup_loss = maximize_triangular_ratio(Lz)            

        loss_tensor = torch.cat([l.unsqueeze(0) for l in [ae_loss, ml_dct_loss, filter_loss, pose_loss, pose_im_loss, im_smooth_unsup_loss, im_triangular_sup_loss]])
        ...
        if is_train:
            ...
            f = (batch_idx + 1, len(dl), *metrics.get_train_values_as_display(), np.round(task_weights.detach().cpu().numpy(), decimals=6))
            print('{}/{}: AE: {}, ML DCT: {}, Filter loss: {}, Pose loss: {}, Pose IM loss: {}, IM smoothing U: {}, IM lower triangle: {}, Loss: {}, vars: {}'.format(*f), end='\r', sep='')
        else:
...
if __name__ == '__main__':
    ...
    with open('config.yaml', 'r') as f:
        ...
        save_dir = Path(project_conf['model_save_root_dir']) / 'nnimvs'
    ...
    config = {
        ...
        'losses': {
            ...
            'interaction_matrix_smoothing_unsup': (False, 0.00),
            'interaction_matrix_triangular_superior_loss': (False, 0.0),
        },
    ...
    def make_name(_dt, config):
        return f'aevs_mlvs_retrain_perturb'
